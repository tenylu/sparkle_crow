...
export const SharedProxyModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-[1050] bg-black/40 backdrop-blur-sm px-4">
      <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl w-full max-w-sm">
        <div className="px-5 py-3 border-b border-gray-200 dark:border-gray-700">
          <h3 className="text-base font-semibold text-gray-800 dark:text-white">共享代理</h3>
        </div>
        <div className="px-5 py-5 space-y-3.5 text-xs">
          <p className="text-xs text-gray-600 dark:text-gray-400 text-center">局域网设备可通过以下地址连接使用代理</p>
          
          {/* IP Selection */}
          {lanIPs.length > 0 && (
            <div>
              <label className="text-[11px] text-gray-500 dark:text-gray-400 mb-1.5 block">选择本机IP:</label>
              <select 
                value={selectedLanIP}
            ...
            <div className="space-y-2.5">
              ...
              <div className="bg-gray-50 dark:bg-gray-700 rounded-xl p-3">
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-[11px] text-gray-500 dark:text-gray-400">HTTP代理:</span>
                  <button
                    onClick={() => {
              ...
              <div className="bg-gray-50 dark:bg-gray-700 rounded-xl p-3">
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-[11px] text-gray-500 dark:text-gray-400">SOCKS5代理:</span>
                  <button
                    onClick={() => {
              ...
              {!isConnected && (
                <p className="text-[11px] text-orange-500 dark:text-orange-400 text-center">提示：请先连接VPN以启用代理服务</p>
              )}
              
              {!allowLan && isConnected && (
                <div className="bg-red-50 dark:bg-red-900/20 rounded-xl p-3">
                  <p className="text-[11px] text-red-600 dark:text-red-400 text-center font-semibold mb-1">⚠️ 局域网共享未开启</p>
                  <p className="text-[11px] text-red-500 dark:text-red-500/70 text-center">需要在设置中开启“允许局域网连接”才能使用共享代理</p>
                </div>
              )}
              
              {allowLan && isConnected && (
                <p className="text-[11px] text-green-600 dark:text-green-400 text-center">✅ 局域网共享已开启</p>
              )}
            </div>
          ) : (
            <div className="bg-yellow-50 dark:bg-yellow-900/20 rounded-xl p-3 text-center">
              <p className="text-xs text-yellow-600 dark:text-yellow-400">未检测到LAN IP地址</p>
              <p className="text-[11px] text-yellow-500 dark:text-yellow-500/70 mt-1">请确保设备已连接到局域网</p>
            </div>
          )}
        </div>
        <div className="px-5 py-3 border-t border-gray-200 dark:border-gray-700 flex justify-end">
          <button
            ...
            className="px-5 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors text-xs"
          >
            关闭
          </button>
        </div>
